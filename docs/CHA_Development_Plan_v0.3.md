...
> Lewende beplanningsdokument. **Status: v0.3 · 21 Augustus 2026 — vervang v0.2.**
> **Opdatering in v0.3:** geen argitektuur- of skopveranderinge nie — status-opdatering. CHA se Paystack-rekening (die opvolg-rekening vir die Heritage Trail, ná die reeds-lewendige CFoL-rekening) het sandbox-toetsing afgehandel en die aktiveringsversoek is by Paystack ingedien. **Ons wag nou vir Paystack se aktivering** voordat regte (lewende) transaksies deur `/checkout` → `/verify-token` verwerk kan word. Sien §7 en §11.
>
...
- **Wetlike bladsye (opgedateer 20–21 Aug 2026):** Privacy Policy, Terms of Service, en Refund and Cancellation Policy is aangepas vanaf GRHS se sjablone en **lewend gepubliseer** op `clarensheritage.org` (`/privacy-policy/`, `/terms-of-service/`, `/refund_returns/`), met skakels in die webwerf-voetskrif. CHA word as die verantwoordelike party genoem; Acutus Consulting as die ontwikkelaar. Sien `CHA_Legal_Docs_Adaptation_v1.md` vir volle besonderhede.
...
**Status (21 Aug 2026): ons wag vir Paystack se aktivering.** Paystack se eie besigheidsvertoningsproses (dieselfde een wat reeds vir die Clarens Festival of Lights-rekening deurloop en goedgekeur is) is nou gevolg vir CHA se Heritage Trail-rekening:

- Sandbox-/toets-modus-integrasie is afgehandel.
- 'n Aktiveringsversoek is by Paystack ingedien, met verwysing na die reeds-lewendige CFoL-rekening, en met die vereiste bewyse aangeheg (voorbeeldprojek, verwysing na kliëntkommunikasie, en skakels na die nou-lewende Privacy Policy / Terms of Service / Refund and Cancellation Policy op `clarensheritage.org`, wat presies ooreenstem met wat Paystack in die vorige hersiening gevra het).
- **Blokkeer:** 'n regte (lewende) Paystack-transaksie deur `/checkout` → `/verify-token` (§4, §9 week 4) kan eers loop sodra Paystack bevestig die rekening is lewend. Sandbox-toetsing met toets-sleutels kan intussen voortgaan en blokkeer nie ander argitektuurwerk nie.
- Sodra Paystack aktivering bevestig: vervang die toets-sleutels met die lewende geheime sleutel in `.env`, en voer die regte sandbox→lewende-oorgangstoets in §9 week 4 uit.
...
| Week | Fokus |
|---|---|
| 1 (19–26 Aug) | Vurk `/wordpress-plugin` + `/app` na nuwe CHA-kodebasis; meganiese hernoem (`GRHS_*`→`CHA_*`, `grhs_`→`cha_`, `GHT-`→`CHT-`, naamruimte→`cha/v1`); Paystack-subaccount opgestel; wetlike bladsye gepubliseer ✅ |
...
| 4 (10–16 Sep) | End-tot-end-toets: volledige aankoopvloei met regte Paystack-transaksie ⚠️ **hang af van Paystack se aktivering (sien §7)** ; QA oor toestelle; vennote/vouchers ingevoer |
...
5. **Nuut (21 Aug 2026):** geen aksie nodig van Claude se kant terwyl ons op Paystack wag nie — behalwe om te onthou om die `.env`-sleutels op te dateer en die lewende-transaksietoets (§9 week 4) te laat loop sodra Paystack se aktiveringsbevestiging ontvang word. Laat weet wanneer dit deurkom.

---

## 11. Veranderingslog

- **v0.3 (21 Aug 2026)** — Status-opdatering, geen argitektuur-/skopverandering nie. CHA se Paystack-aktiveringsversoek (Heritage Trail-rekening, ná die reeds-lewendige CFoL-rekening) is ingedien ná afhandeling van sandbox-toetsing; **ons wag nou vir Paystack se bevestiging** voordat lewende transaksies verwerk kan word (sien §7, §9 week 4). Prys-oop-vraag (§10.2) as opgelos gemerk. Wetlike bladsye (Privacy Policy/Terms of Service/Refund and Cancellation Policy) se lewende publikasie op `clarensheritage.org` in §6 aangeteken.
